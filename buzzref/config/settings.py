...
class BuzzSettingsEvents(QtCore.QObject):
...
# BuzzSettings instance. Since we can't instantiate BuzzSettings
# globally on module level (because the Qt app doesn't exist yet), we
# use this events proxy
settings_events = BuzzSettingsEvents()


class BuzzSettings(QtCore.QSettings):
    ...
    def valueOrDefault(self, key):
        ...
        opposed to settings that BuzzRef stores on its own.)
